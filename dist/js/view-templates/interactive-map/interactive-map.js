...
(function ($) {
  $.fn.rsInteractiveMap = function initPlugin(opts) {
    ...
    var actions = {
      ...
      buildMap: function buildMap(config) {
        ...
        for (var i = 0; i < circles.length; i += 1) {
          ...
          var $template = $("\n          <div class=\"rsMap-".concat(circle.type, "\" style=\"top:").concat(circle.top, ";left:").concat(circle.left, ";\">\n            <div class=\"rsMap-inner\">\n              <div class=\"rsMap-toolTip\">\n                <span class=\"rsMap-toolTip-header\">").concat(circle.tipTitle, "</span>\n                <p class=\"rsMap-toolTip-body\">").concat(circle.tipBody, "</p>\n                <span class=\"rsMap-toolTip-footer\">").concat(circle.tipFooter, "</span>\n                <div class=\"rsMap-toolTip-arrow rsMap-arrow-bottom\"></div>\n              </div>\n              <div class=\"rsMap-").concat(circle.type, "-circle\">").concat(circle.items, "</div>\n            </div>\n          </div>"));
        ...
        $mobile.insertBefore(this.mapSelector).append("<img src=\"".concat(config.mobileImage, "\" class=\"rsMap-mobileImage\"/>"));
      },
      positionTip: function positionTip($el, type, offScreen) {
        var $tipContainer = $el.find('.rsMap-toolTip');
        var $circle = $el.find(".rsMap-".concat(type, "-circle"));
      ...
      hoverCircle: function hoverCircle($el, type) {
        ...
        var $tipContainer = $el.find('.rsMap-toolTip');
        var $tipArrow = $tipContainer.find('.rsMap-toolTip-arrow'); // reset tip callback
        ...
        hoverintent($el.get(0), function () {
          clearTimeout(_this.hoverTimeout); // hide any open bubbles on the page except active one, then show hovered tip
          ...
          $tipArrow.removeClass('rsMap-arrow-left').addClass('rsMap-arrow-bottom'); // if tip is off screen & covered by nav, reposition to the side.
    ...
    actions.init(options, $map); // for testing usage
