...
(function ($) {
  $(function () {
    var $modalSource = $('.rackspaceModalSource'); // Set default modal settings.
    ...
    var positionWindow = function positionWindow() {
      ...
      $modalSource.dialog('option', 'maxHeight', modalMaxHeight).dialog('option', 'width', modalWidth).dialog('option', 'position', {
        my: 'center',
        at: 'center',
        of: window
      });
    ...
    $(window).resize(positionWindow); // Opens the specified modal.
    ...
    var openModal = function openModal(selector) {
      ...
      $el.dialog('open');
      positionWindow(); // Output modal options for copy/pasting into Drupal.
