...
;( function( window ) {
//search. not sure how much we need yet. shoud review together.
	'use strict';
...
//mobile check
	...
	function UISearch( el, options ) {
		this.el = el;
		this.inputEl = el.querySelector( 'form > input.navbar-search-input' );
		this._initEvents();
	}
	...
	UISearch.prototype = {
		_initEvents : function() {
			var self = this,
				initSearchFn = function( ev ) {
					...
					if( !classie.has( self.el, 'navbar-search-open' ) ) { // open it
						ev.preventDefault();
						self.open();
					}
					else if( classie.has( self.el, 'navbar-search-open' ) && /^\s*$/.test( self.inputEl.value ) ) { // close it
						ev.preventDefault();
						self.close();
					}
				}
		...
		open : function() {
			var self = this;
			classie.add( this.el, 'navbar-search-open' );
			// focus the input
			if( !mobilecheck() ) {
				this.inputEl.focus();
			}
		...
		close : function() {
			this.inputEl.blur();
			classie.remove( this.el, 'navbar-search-open' );
		}
	}
